/*

Prompt a user for their name and then display a greeting message that repeats the name a specified number of times.  Practice using concat and join.

You may also need some of the following (but not required): 
- parseInt
- isNaN
- new Array()

*/

// Do not remove the below line
const prompt = require("prompt-sync")();
// Do not remove the above line

function askForNameAndRepeat() {
  // Convert the repeat count to an integer if it's not already
  let askName = prompt('whats your name?')
  let getRepCount = prompt('how many times should i repeat your name?')
  let parsedRepCount = parseInt(getRepCount)
  // Check if the repeat count is a number and greater than zero
  if(parsedRepCount > 1) {
    // Create an array filled with the user's name, repeated the desired number of times
    return askName.repeat(parsedRepCount)
  } else {
    return 'number is not 1 or greater'
  }
  // Use concat to combine the repeated names into one string
}

askForNameAndRepeat();